myEval <- function (frame, outstrata, nsampl = 100, cens = NULL, writeFiles = TRUE, 
	progress = TRUE) 
{
	if (writeFiles == TRUE) {
		dire <- getwd()
	...
	for (i in (3:ncol(diff))) {
		stmt <- paste("bias$y", i - 2, " <- tapply(diff$diff", 
			i - 2, ",diff$dom,mean)", sep = "")
		eval(parse(text = stmt))
	}
	bias$dom <- paste("DOM", c(1:numdom), sep = "")
	bias <- as.data.frame(bias)
	Y <- aggregate(frame[, grep("Y", colnames(frame))], by = list(frame$DOMAINVALUE), 
		mean)
	...
	if (writeFiles == TRUE) {
		write.table(est, "estimates.csv", sep = ",", row.names = F, 
			col.names = F)
	}
	cv <- round(cv[, c(1:numY)], 4)
	cv <- cbind(c(1:numdom), cv)
	colnames(cv) <- c("domain", paste("cv(Y", c(1:numY), ")", 
		sep = ""))
	bias <- cbind(c(1:numdom), bias)
	colnames(bias) <- c("domain", paste("bias(Y", c(1:numY), 
		")", sep = ""))	
