...
//Create sctnaics table for SCTG NAICS mapping data
...
/* Aggregate value, weight, and mile by state and SCTG in faf16 tables
...
select state, naics, weight, value, miles into faf16s from faf16s inner join sctgnaics on faf16s.sctg = sctgnaics.sctg;
...
select cbpf.state, cbpf.county, cbpf.naics, (cbpf.est*fafbp.weight)/fafbp.est as weight, (cbpf.est*fafbp.value)/fafbp.est as value, 
(cbpf.est*fafbp.miles)/fafbp.est as miles, cbpf.ap as ap, cbpf.est as est into fafcbp from cbpf 
inner join fafbp on cbpf.state = fafbp.state and fafbp.naics = cbpf.naics;