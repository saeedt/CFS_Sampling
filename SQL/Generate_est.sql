...
/*Usage:
generate_est(Sample_size, Source_table, Value_CV, Mile_CV)
Example: 
SELECT * FROM generate_est(100000, 'fafcbp, 0.5, 0.5);
*/

drop function if exists generate_est(integer,text,numeric,numeric);
...
CREATE OR REPLACE FUNCTION generate_est(_psize int = 1000, _source text = 'fafcbp', _ValCV numeric = 0.5 , _MilCV numeric = 0.5)
